...
const FriendRequestList = () => {
      ...
      <div className="space-y-3 px-2">
        {[1, 2].map((i) => (
          <div key={i} className="flex gap-3 animate-pulse items-center p-2">
            <div className="w-14 h-14 bg-bg rounded-full"></div>
            <div className="flex-1 space-y-2">
              <div className="h-3 bg-bg rounded w-1/2"></div>
              <div className="h-8 bg-bg rounded w-full"></div>
            </div>
          </div>
        ))}
      </div>
  ...
  if (requests.length === 0) {
    return (
      <div className="mx-2 p-4 text-center rounded-xl border border-gray-800/50 bg-bg">
